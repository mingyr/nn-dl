...
images, _ = input_("data/MNIST/mnist-train.tfr")
...
# 计算损失函数
with tf.control_dependencies([tf.assert_equal(tf.rank(images), tf.rank(restored))]):
    loss = tf.math.reduce_mean(tf.math.squared_difference(images, restored))
